...
void Background::createDefaultRect() 
{
	...
	colorStripDstRect[score] = setScreenUnitRect(-2, 9 * currentColorPlace, 10, 9);
...
void Title::createDefaultRect()
{
	...
	titleDstRect[toInt(TITLE::SCORE)] = setScreenUnitRect(9.5, 1, 5, 1.5);
}
...
void SettingTexture::createDefaultRect() {
	...
	for (int type = 0; type < toInt(CONTROL::TOTAL); type++) {
		buttonDstRect[type] = setTextRect(1, static_cast<double>(type * 2 + 3) / 3.0, button[type].getWidth(), button[type].getHeight());
	}
}
...
void Button::createSettingRect() {
	...
	for (int type = 0; type <= toInt(BUTTON::EXIT); type++) {
		usingButton[type] = true;
	}
	usingButton[saveNumb] = false;
}

void Button::createScoreRect() {
	int exitNumb = toInt(BUTTON::EXIT);

	buttonDstRect[exitNumb] = setScreenUnitRect(10.5, 7.5, 3, 1);
	usingButton[exitNumb] = true;
}
...
void Point::renderPointScreen() {
	nameTexture->render(renderer, nameRect, NULL);
...
void ArrowTexture::renderPressedArrow(Event& event, Point& point)
{
	for (int type = 0; type < toInt(CONTROL::TOTAL_ARROW); type++) {
		if (event.checkKeyState(static_cast<CONTROL>(type))) {
			...
			if (event.e.key.repeat == 0 && event.checkKeyEventDown(static_cast<CONTROL>(type))) {
